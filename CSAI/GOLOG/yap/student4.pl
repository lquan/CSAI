...
poss(solveTopic(T),S) :- researchDone(T,S), not taskSolved(T,S).%taskGiven(T,S),
poss(sendSolution(T),S) :- taskSolved(T,S), taskGiven(T,S).


taskGiven(T,do(A,S)) :- taskGiven(T,S), not A = sendSolutionS(T).
researchDone(T,do(A,S)) :- researchDone(T,S); not researchDone(T,S), A = researchTopicS(T). %researchDone(T,S) ; A = researchTopicS(T). %
%taskSolved(T,do(A,S)) :- taskSolved(T,S) ; A = solveTopicS(T), not taskSolved(T,S).
