...
poss(solveTopicS(T),S) :- researchDone(T,S), not taskSolved(T,S),taskGiven(T,S).
poss(solveTopicF(T),S) :- researchDone(T,S), not taskSolved(T,S),taskGiven(T,S).
poss(sendSolutionS(T),S) :- taskSolved(T,S), taskGiven(T,S).
poss(sendSolutionF(T),S) :- taskSolved(T,S), taskGiven(T,S).
...
taskDelivered(T,do(A,S)) :- taskDelivered(T,S) ; A = sendSolutionS(T).
